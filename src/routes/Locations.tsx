...
export function Locations() {
  ...
  const [editing, setEditing] = useState<LocationRead | undefined>()
  const [deleteTarget, setDeleteTarget] = useState<LocationRead | null>(null)
  ...
  const openEdit = (loc: LocationRead) => {
    setEditing(loc)
    setFormOpen(true)
  }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-2xl font-bold">儲位管理</h2>
          ...
          <TableBody>
            ...
            {locations.map(loc => (
              <TableRow key={loc.id}>
                ...
                <TableCell>
                  {new Date(loc.created_at).toLocaleString('zh-TW')}
                </TableCell>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>確定要刪除這筆儲位?</AlertDialogTitle>
            <AlertDialogDescription>
